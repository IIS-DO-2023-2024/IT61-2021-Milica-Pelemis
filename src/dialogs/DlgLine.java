...
public class DlgLine extends JDialog {
	...
	private void createHeader() {
		...
		JLabel lblSubtitle = new JLabel("Enter start point, end point and edge color");
	...
	private void createContent() {
		...
		contentPanel.setBorder(
				BorderFactory.createCompoundBorder(new LineBorder(borderColor, 1, true), new EmptyBorder(18, 20, 18, 20)));

		txtStartX = createTextField();
		addInputRow(contentPanel, "Start X", txtStartX, 0);

		txtStartY = createTextField();
		addInputRow(contentPanel, "Start Y", txtStartY, 1);

		txtEndX = createTextField();
		addInputRow(contentPanel, "End X", txtEndX, 2);

		txtEndY = createTextField();
		addInputRow(contentPanel, "End Y", txtEndY, 3);
		...
		btnEdgeColor.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Color selectedColor = JColorChooser.showDialog(DlgLine.this, "Choose edge color", color);

				if (selectedColor != null) {

					color = selectedColor;
					updateColorButton(btnEdgeColor, color);
				}
			}
		});
